...
@onready var growt_cycle_component: GrowtCycleComponent = $GrowtCycleComponent
...
func _ready() -> void:
	...
	growt_cycle_component.crop_maturity.connect(on_crop_maturity)
	growt_cycle_component.crop_harvesting.connect(on_crop_harvesting)

func _process(delta: float) -> void:
	growth_state = growt_cycle_component.get_current_growth_state()
...
func on_hurt(hit_damage: int) -> void:
	if !growt_cycle_component.is_watered:
		watering_particles.emitting = true
		await get_tree().create_timer(5.0).timeout
		watering_particles.emitting = false
		growt_cycle_component.is_watered = true
